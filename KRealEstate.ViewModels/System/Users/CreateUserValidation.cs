...
namespace KRealEstate.ViewModels.System.Users
{
    public class CreateUserValidation : AbstractValidator<UserCreateRequest>
    {
        public CreateUserValidation()
        {
            ...
            //RuleFor(x => x.Password).NotEmpty().WithMessage("Password is required!!").Matches(@"?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[^\w\s])^.{10,}$").WithMessage("Mật khẩu phải có ít nhất 1 chữ in hoa, ký tự đặc biệt")
